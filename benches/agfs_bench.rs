...
fn bench_agfs_write(c: &mut Criterion) {
    ...
    group.bench_function("write_1kb", |b| {
        ...
        b.iter(|| {
            ...
            fs.write(&path, &data, 0, WriteFlag::CREATE).expect("Write failed");
            counter += 1;
            black_box(&fs);
        });
    });
...
fn bench_agfs_read_dir(c: &mut Criterion) {
    ...
    for i in 0..100 {
        ...
        fs.write(&path, b"data", 0, WriteFlag::CREATE).expect("Write failed");
    }
...
fn bench_agfs_stat(c: &mut Criterion) {
    ...
    for i in 0..1000 {
        ...
        fs.write(&path, b"test data", 0, WriteFlag::CREATE).expect("Write failed");
    }
...
fn bench_agfs_exists(c: &mut Criterion) {
    ...
    for i in 0..500 {
        ...
        fs.write(&path, b"x", 0, WriteFlag::CREATE).expect("Write failed");
    }
...
fn bench_agfs_append(c: &mut Criterion) {
    ...
    // Create initial file
    fs.write("/append_test", b"initial", 0, WriteFlag::CREATE).expect("Write failed");
    ...
    group.bench_function("append_100_bytes", |b| {
        ...
        b.iter(|| {
            fs.write("/append_test", &data, 0, WriteFlag::APPEND).expect("Append failed");
            black_box(&fs);
        });
    });
