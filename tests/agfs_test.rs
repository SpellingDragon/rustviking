...
fn test_write_and_read_through_agfs() {
    ...
    // Read through AGFS
    let data = agfs.route_operation("/test/hello.txt", |fs| {
        fs.read("/test/hello.txt", 0, 0)
    }).unwrap();
...
fn test_mkdir_and_list() {
    ...
    agfs.route_operation("/test/mydir", |fs| {
        fs.mkdir("/test/mydir", 0o755)
    }).unwrap();
    ...
    let entries = agfs.route_operation("/test/mydir", |fs| {
        fs.read_dir("/test/mydir")
    }).unwrap();
