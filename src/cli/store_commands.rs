...
pub fn exec_kv_put(store: &dyn KvStore, key: &str, value: &str, _format: &OutputFormat) -> Result<()> {
...
pub fn exec_kv_scan(store: &dyn KvStore, prefix: &str, limit: usize, _format: &OutputFormat) -> Result<()> {
    let results = store.scan_prefix(prefix.as_bytes())?;
    let entries: Vec<serde_json::Value> = results.into_iter()
        .take(limit)
        .map(|(k, v)| {
            serde_json::json!({
